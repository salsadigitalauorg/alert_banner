...
(function ($, Drupal) {
  ...
  Drupal.behaviors.AlertsRestBlock = {
    attach: function (context, settings) {
      // Set alert text color.
      $('.alert-banners article.node--type-alert', context).each(function (index, element) {
      ...
      $('.alert-banners:not(.alerts-processed)', context).once('alerts_load').each(function (index, element) {
        var endpoint = $(element).attr('data-alert-endpoint');
        if ((typeof endpoint == 'undefined') || !endpoint || endpoint.length === 0) {
          endpoint = '/alerts?_format=json';
        }
        $.getJSON(endpoint, function (response) {
          if (response.length) {
            ...
            for (var i = 0, len = response.length; i < len; i++) {
              ...
              // Build the alert.
              var $alert = $('<article role="article" data-alert-id="' + alert_item.alert_id + '" class="node node--type-alert"><div class="layout-container node__content"></div></article>');
              ...
              if (typeof alert_item.message !== 'undefined') {
                ...
                $('<div class="field clearfix alert-message">' + alert_message + '</div>')
                  .appendTo($alert.find('.node__content'));
              }

              // Attaches the link.
              if (typeof alert_item.link !== 'undefined' && alert_item.link !== false && alert_item.link !== "") {
                $('<div class="field clearfix alert-link">' + alert_item.link + '</div>')
                  .appendTo($alert.find('.node__content'));
              }
              else {
                $alert.addClass('alert-link--none');
              }
              ...
              if ((typeof alert_item.permanent !== 'undefined') && (alert_item.permanent !== '1')) {
                ...
                $('<button class="alert-close" data-alert-id="' + alert_item.alert_id + '" aria-label="' + label_dismiss + '"><span>' + label_dismiss + '</span></button>')
                  .appendTo($alert.find('.node__content'));
              }
              else {
            ...
            Drupal.behaviors.AlertsRestBlock.attach(context, settings);
          }
        });
      });
    }
